...
router.get('/', authenticate, (req, res) => {
  const rows = db.prepare('SELECT id, subject_id, original_name, uploader, date FROM notes ORDER BY date DESC').all();
  res.json(rows);
});
...
router.post('/', authenticate, requireRole('staff'), uploadPdf.single('file'), (req, res) => {
  ...
  if (!subjectId) return res.status(400).json({ error: 'Choose a subject for this note.' });
  ...
  db.prepare('INSERT INTO notes (id, subject_id, original_name, stored_name, uploader, date) VALUES (?,?,?,?,?,?)')
    .run(row.id, row.subject_id, row.original_name, row.stored_name, row.uploader, row.date);
  res.json({ id: row.id, subject_id: row.subject_id, original_name: row.original_name, uploader: row.uploader, date: row.date });
});
...
router.put('/:id/rename', authenticate, requireRole('staff'), (req, res) => {
  ...
  const info = db.prepare('UPDATE notes SET original_name = ? WHERE id = ?').run(name.trim(), req.params.id);
...
router.put('/:id/replace', authenticate, requireRole('staff'), uploadPdf.single('file'), (req, res) => {
  ...
  db.prepare('UPDATE notes SET original_name=?, stored_name=?, date=? WHERE id=?')
